...
TEXT_NOTIFICATION_TEXT = ('One or more of your leases is at risk of closing today, tomorrow, or in 2 days. '
                          'Visit go.ncsu.edu/shellcast for details.')
...
def sendNotificationsWithAWSSES(emailNotifications, textNotifications):
    ...
    for address, emailBodyChunks, userId in emailNotifications:
        ...
        time.sleep(
            EMAIL_SEND_INTERVAL)  # add a delay so that we don't exceed our max send rate (currently 14 emails/second)
...
def sendNotifications():
    ...
    for address, notificationText, notificationType, userId, sendSuccess, resText in responses:
        notification = Notification(address=address, notification_text=notificationText,
                                    notification_type=notificationType, user_id=userId, send_success=sendSuccess,
    ...
    result = 'Constructed and sent {} email notifications and {} text notifications to {} users in {} seconds'.format(
        len(emailNotificationsToSend), len(textNotificationsToSend), len(users), (t1 - t0) / 1000000000)
    logging.info(result)
    return result
