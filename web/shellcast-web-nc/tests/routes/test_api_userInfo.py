...
def test_valid(client, dbSession, addMockFbUser):
    ...
    provider = PhoneServiceProvider(
        name="Bleh Mobile", mms_gateway="mms.blah.com", sms_gateway="sms.blah.com"
    )
...
def test_update_info(client, addMockFbUser, dbSession):
    ...
    res = client.post(
        "/userInfo", headers={"Authorization": "validUser1"}, json=updateData
    )
    print(res.get_json())
    assert res.status_code == 200
...
def test_update_invalid_email(client, addMockFbUser):
    ...
    res = client.post(
        "/userInfo", headers={"Authorization": "validUser1"}, json={"email": "myemail"}
    )
...
def test_update_invalid_phone(client, addMockFbUser, dbSession):
    ...
    res = client.post(
        ...
        json={"phone_number": "123456789", "service_provider_id": provider.id},
    )
    assert res.status_code == 400
