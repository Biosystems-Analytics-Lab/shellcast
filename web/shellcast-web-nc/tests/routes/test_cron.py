...
def test_sendNotifications(client, dbSession, monkeyPatchBotoClient):
    ...
    provider = PhoneServiceProvider(
        name="Verizon", mms_gateway="vzwpix.com", sms_gateway="vtext.com"
    )
    ...
    assert (
        ...
        in results[0].notification_text
    )
    ...
    assert (
        "One or more of your leases has a high percent chance of closing today, tomorrow, or in 2 days. Visit go.ncsu.edu/shellcast for details."
        == results[1].notification_text
    )
