...
STATE = "SC"
# Bandwidth SMS message template (SC has 3-day forecast like NC)
    ...
    for user in users:
        ...
        for user_lease in user.leases:
            if user_lease.deleted:
                continue
            prob = user_lease.get_latest_probability()
            if prob:
                if (
...
def send_bandwidth_message():
    ...
    logging.info("Starting Bandwidth SMS notification send for SC...")
    ...
    result = f"SC: Sent {success_count} SMS notifications ({fail_count} failed) in {(t1 - t0) / 1e9:.2f} seconds"
    logging.info(result)
