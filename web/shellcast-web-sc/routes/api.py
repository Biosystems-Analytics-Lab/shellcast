...
def _handle_inbound_message_sc(from_number, text, message_id):
    """
    Handle incoming SMS from user (STOP, HELP) for SC.
    Updates SC's users table for opt-out.
    """
    ...
    # Handle HELP keyword - send help message
        ...
        from routes.cron import _send_bandwidth_message

        logging.info(f"Sending help message to {from_number}")
        try:
            _send_bandwidth_message([from_number])
