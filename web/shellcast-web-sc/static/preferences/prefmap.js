...
const LEGEND_SCALE = [{text: 'No data', color: COLOR_NULL}, {text: 'Very Low', color: COLOR_VERY_LOW}, {
    text: 'Low', color: COLOR_LOW
}, {text: 'Moderate', color: COLOR_MODERATE}, {text: 'High', color: COLOR_HIGH}, {
...
let popupOverlay;
// Lease place marker
let markerSvg = '<svg id="marker" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="30px" height="30px" viewBox="0 0 30 30" enable-background="new 0 0 30 30" xml:space="preserve">' + '<path fill="white" stroke="black" d="M22.906,10.438c0,4.367-6.281,14.312-7.906,17.031c-1.719-2.75-7.906-12.665-7.906-17.031S10.634,2.531,15,2.531S22.906,6.071,22.906,10.438z"/>' + '<circle fill="black" cx="15" cy="10.677" r="3.291"/></svg>';
...
async function initMap(growingUnitData) {
    ...
    cmuLyr = new ol.layer.Vector({
        title: 'CMU', name: 'cmu', opacity: 0.9, source: cmuGeoJsonSource,
    }); // #3
    ...
    //         url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', // maxZoom: 8
    //     })
    // }); // #6
...
async function createLeasePointFeatures(leaseData) {
    let features = leaseData.map(item => {
        ...
        let longitude = item.longitude,
            ...
            iconStyle = new ol.style.Style(/** @type {olx.style.IconOptions} */ {
