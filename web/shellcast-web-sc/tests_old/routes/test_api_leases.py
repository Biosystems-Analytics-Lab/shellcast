...
def test_get_leases(client, dbSession, addMockFbUser):
  ...
  leases = [
    UserLease(user_id=user.id, lease_id='45678', grow_area_name='A01', cmu_name='U001', rainfall_thresh_in=1.5, geometry=(34.404497, -77.567573)),
    UserLease(user_id=user.id, lease_id='12345', grow_area_name='B02', cmu_name='U002', rainfall_thresh_in=2.5, geometry=(35.207332, -76.523872)),
    UserLease(user_id=user.id, lease_id='82945', grow_area_name='C01', cmu_name='U003', rainfall_thresh_in=1.5, geometry=(36.164344, -75.927864))
  ]
...
def test_add_lease(client, dbSession, addMockFbUser):
  ...
  lease = UserLease(user_id=user.id, lease_id='45678', grow_area_name='A01', cmu_name='U001', rainfall_thresh_in=1.5, geometry=(34.404497, -77.567573))
  dbSession.add(lease)
  dbSession.commit()

  # add some NCDMF leases
  ncdmfLeases = [
    Lease(lease_id='819401', grow_area_name='D11', cmu_name='U011', rainfall_thresh_in=2.5, geometry=(34.404497, -77.567573)),
    Lease(lease_id='123456', grow_area_name='B05', cmu_name='U021', rainfall_thresh_in=3.5, geometry=(35.923741, -76.239482)),
    Lease(lease_id='4-C-89', grow_area_name='A01', cmu_name='U031', rainfall_thresh_in=1.5, geometry=(36.303915, -75.864693))
  ]
  dbSession.add_all(ncdmfLeases)
  dbSession.commit()

  # make a request to add the given lease
  res = client.post('/leases', headers={'Authorization': 'validUser1'}, json={'lease_id': '4-C-89'})
  assert res.status_code == 200

  json = res.get_json()

  assert json['lease_id'] == '4-C-89'
...
def test_add_invalid_lease(client, dbSession, addMockFbUser):
  ...
  # make a request to add the given lease
  res = client.post('/leases', headers={'Authorization': 'validUser1'}, json={'lease_id': '123-CD'})
