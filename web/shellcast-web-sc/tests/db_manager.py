...
class TestDatabaseManager:
    ...
    def __init__(self, app, config):
        ...
        if hasattr(config, 'SQLALCHEMY_DATABASE_URI'):
            ...
            self.original_uri = getattr(config, 'SQLALCHEMY_DATABASE_URI', 'sqlite:///:memory:')
        self.test_uri = None
...
class TestCase:
    ...
    def assertDatabaseHas(self, model_class, **filters):
        ...
        assert record is not None, f"No {model_class.__name__} found with filters: {filters}"
    ...
    def assertDatabaseCount(self, model_class, expected_count):
        ...
        assert actual_count == expected_count, f"Expected {expected_count} {model_class.__name__} records, got {actual_count}"
